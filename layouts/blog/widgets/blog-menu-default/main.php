	...
	function getMenuWidgetTree($site,$table,$is_child,$parent,$depth,$id,$w,$_C)
	{
		...
		if ($depth < $w['limit'])
		{
			...
			while($C=db_fetch_array($CD))
			{
				...
				if ($_dropdown) echo '<a class="blog-nav-item'.(in_array($C['id'],$_CA)?' active':'').'"'.$_addattr.$_href.$_dropdown.$_target.'>'.$_name.'</a>';
				else echo '<a class="blog-nav-item'.(in_array($C['id'],$_CA)?' active':'').'"'.$_addattr.$_href.$_dropdown.$_target.'>'.$_name.'</a>';

				if ($C['is_child'])
				{
					$C['link'] = '<li><a'.$_addattr.$_href.$_target.'>'.$C['name'].'</a>';
