	...
	function getMenuWidgetTree($site,$table,$is_child,$parent,$depth,$id,$w,$_C)
	{
		...
		if ($depth < $w['limit'])
		{
			...
			while($C=db_fetch_array($CD))
			{
				...
				if ($_dropdown) echo '<li '.(in_array($C['id'],$_CA)?' class="active"':'').'><a '.$_addattr.$_href.$_dropdown.$_target.'>'.$_name.'</a></li>';
				else echo '<li'.(in_array($C['id'],$_CA)?' class="active"':'').'><a '.$_addattr.$_href.$_dropdown.$_target.'>'.$_name.'</a></li>';
